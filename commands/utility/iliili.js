...
async function pokreniTurnir(interaction, channelId, slike, igraci) {
  ...
  for (let runda = 0; runda < 6; runda++) {
    ...
    const rundaEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `**Prva devojka** vs **Druga devojka**\n\nGlasajte ispod! ⏳ **7 sekundi!**`
      )
      .setImage(prva)
      .setTimestamp();
 
    // Pošalji prvu sliku
    const prvaMsg = await channel.send({
      content: "⬅️ **Prva devojka**",
      files: [prva],
    }).catch(() => null);
 
    // Pošalji drugu sliku
    const drugaMsg = await channel.send({
      content: "➡️ **Druga devojka**",
      files: [druga],
    }).catch(() => null);
    ...
    const glasanjeRow = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("prva")
        .setLabel("⬅️ Prva devojka")
        .setStyle(ButtonStyle.Primary),
      new ButtonBuilder()
        .setCustomId("druga")
        .setLabel("➡️ Druga devojka")
        .setStyle(ButtonStyle.Primary)
    );
    ...
    const glasovi = { prva: new Set(), druga: new Set() };
    ...
    kolektor.on("collect", async (btn) => {
      ...
      glasovi.prva.delete(btn.user.id);
      glasovi.druga.delete(btn.user.id);
      // Dodaj novi glas
    ...
    const disabledGlasanje = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("prva")
        .setLabel(`⬅️ Prva (${glasovi.prva.size})`)
        .setStyle(ButtonStyle.Primary)
        .setDisabled(true),
      new ButtonBuilder()
        .setCustomId("druga")
        .setLabel(`➡️ Druga (${glasovi.druga.size})`)
        .setStyle(ButtonStyle.Primary)
        .setDisabled(true)
    );
    ...
    const prvaGlasovi = glasovi.prva.size;
    const drugaGlasovi = glasovi.druga.size;
    ...
    if (prvaGlasovi >= drugaGlasovi) {
      ...
      await channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0x2ecc71)
            .setTitle(`✅ Prva devojka ide dalje! (${prvaGlasovi} vs ${drugaGlasovi})`)
            .setDescription(
              runda < 5
      ...
      await channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0x2ecc71)
            .setTitle(`✅ Druga devojka ide dalje! (${drugaGlasovi} vs ${prvaGlasovi})`)
            .setDescription(
              runda < 5
                ? "➡️ Sledeća runda počinje za trenutak..."
                : "🏆 **FINALE!**"
